...
//copy constructed func
template <typename K, typename M, typename H>
HashMap<K,M,H>::HashMap(const HashMap<K,M,H>& other)
{
    this->_size = other._size;
    this->_hash_function = other._hash_function;
    this->_buckets_array = bucket_array_type(other.bucket_count(),nullptr);
    for(size_t i=0;i<other.bucket_count();++i)
    {
        if(other._buckets_array[i]!=nullptr)
        {
            auto p = other._buckets_array[i];
            while(p!=nullptr)
            {
                auto temp = new node(p->value,this->_buckets_array[i]);
                this->_buckets_array[i] = temp;
                p = p->next;
            }
        }
    }
...
//move constructed func
template <typename K, typename M, typename H>
HashMap<K,M,H>::HashMap(HashMap<K,M,H>&& other)
{
    this->_size = other._size;
    this->_hash_function = other._hash_function;
    for(auto& p : other._buckets_array)
    {
        this->_buckets_array.push_back(p);
    }
}
